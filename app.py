...
app.secret_key = '1234'  # Ключ для сессий
user_db = "kirill"
host_ip = "127.0.0.1"
host_port = "5432"
...
migrate = Migrate(app, db) ## falsk db init  ##flask db upgrade
...
def session_details(cinema_session_id):
    ...
    client = [getattr(session_data, f'client_{i}') for i in range(1, 31)]

    return render_template('sessionDetails.html', session_data=session_data, seats=seats, client=client)
...
def reserve_seats(cinema_session_id):
    ...
    user_reserved_seats = sum(getattr(session_data, f'seat_{i}') for i in range(1, 31) if getattr(session_data, f'client_{i}') == session['name'])

    for selected_seat in selected_seats:
        seat_number = int(selected_seat.split('0')[-1])
        if getattr(session_data, selected_seat, False):
            if is_admin:
                setattr(session_data, selected_seat, False)
                setattr(session_data, f'client_{seat_number}', None)
                db.session.commit()
            ...
            if user_reserved_seats + len(selected_seats) <= 5:
                ...
                setattr(session_data, f'client_{seat_number}', session['name'])
                db.session.commit()
...
def cancel_reservation(cinema_session_id, seat_number):
    ...
    elif getattr(session_data, f'client_{seat_number}') != session['name']:
